...
# Stage 2: Data inspection and stepwise screening
#
# PURPOSE:
#   Implements the nine-step screening protocol from Palmer & Strobeck (1994,
#   2003) to produce a clean dataset suitable for FA hypothesis testing.
#   Steps proceed in order. Human review is required at each checkpoint before
#   continuing. All decisions made at checkpoints are recorded in fa-constants.R.
#
# DEPENDENCIES:
#   fa-functions.R  -- all analytical functions (source first)
#   fa-constants.R  -- file paths, group definitions, and decision records
#   Output of 1-wrangling.R (f_wrangled, defined in fa-constants.R)
#
# STEP OVERVIEW (Palmer & Strobeck 1994, 2003):
#   Steps 1-2 : ME outlier detection (Dixon's test on replicate differences
#               of side differences)
#   Steps 3-5 : FA outlier detection (L vs R plots, cross-trait plots,
#               Dixon's test on signed side differences)
#   Step 6    : FA > ME test (Sides x Individuals mixed-model ANOVA)
#   Step 7    : ME variance homogeneity (Levene's test)
#   Step 8    : Trait size dependency (Spearman correlation)
#   Step 9    : Ideal FA assessment (DA, skew, kurtosis)
#
# HUMAN CHECKPOINTS -- after each, update fa-constants.R and re-source:
#   After Steps 1-2 : populate me_dixon_removals
#   After Steps 3-5 : populate fa_dixon_removals
#   After Step 6    : populate step6_eliminations
#   After Step 8    : set ln_transform flag
#   After Step 9    : populate step9_highDA, then step9_eliminations
#
# USAGE:
#   1. Ensure 1-wrangling.R has been run and f_wrangled exists.
#   2. Populate fa-constants.R with your group and trait definitions.
#   3. Run sections sequentially, pausing at each checkpoint.
#   4. After each checkpoint, update fa-constants.R, save, and re-source.
#   5. Proceed to 3-analysis.R after Step 9 removals are complete.
...
######################## SOURCE SHARED FILES
source("fa-functions.R")
source("fa-constants.R")
...
######################## STEPS 1-2: ME OUTLIER DETECTION
...
#
# STEP 1 NOTE:
#   Palmer & Strobeck (2003) Step 1 involves visual inspection of replicate-
#   pair plots. For large datasets this is impractical. Dixon's test in Step 2
#   provides the statistical equivalent. For small samples, consider plotting
#   replicate-pair |R-L| values manually to supplement the test.
#
...
#   3. Add confirmed removals to me_dixon_removals in fa-constants.R.
#      Removal unit: one specific replicate for one individual x trait.
#
# After populating me_dixon_removals, save fa-constants.R and continue.
...
#   3. Add confirmed removals to fa_dixon_removals in fa-constants.R.
#      Removal unit: one specific replicate for one individual x trait.
#
# After populating fa_dixon_removals, save fa-constants.R and continue.
...
# Re-source fa-constants.R to pick up me_dixon_removals and fa_dixon_removals.
...
#   Add it to step6_eliminations in fa-constants.R.
...
# Re-source fa-constants.R to pick up step6_eliminations.
...
for (entry in step6_eliminations) {
    ...
    posts6$Value[mask & posts6$Trait == entry$trait] = NA
}
...
me_var = read_csv(f_fromStep5, show_col_types = FALSE) |>
    filter(!is.na(Value)) |>
    arrange(Ind, across(all_of(group_cols)), Trait, Side, Rep) |>
    group_by(Ind, across(all_of(group_cols)), Trait, Side) |>
    summarise(
...
#   Record your decision as ln_transform = TRUE or FALSE in fa-constants.R.
#
...
#   Set ln_transform = TRUE in fa-constants.R.
...
#   Leave ln_transform = FALSE in fa-constants.R.
#
# Save fa-constants.R before continuing.
...
# step9_highDA section in fa-constants.R for the Palmer & Strobeck (2003)
# rule of thumb on when DA is small enough to retain a trait.
#
# Populate step9_highDA in fa-constants.R with all traits showing
# significant DA. Save fa-constants.R before running the DA assessment below.
...
# must be excluded.
#
# Populate step9_eliminations in fa-constants.R combining DA exceedances,
# skew flags, and kurtosis flags. Save fa-constants.R before continuing.



######################## STEP 9 REMOVALS
# Removes all traits flagged for DA exceedance, skew, or kurtosis.
# Removal unit: entire trait column for a group (Value set to NA).
# Re-source fa-constants.R to pick up step9_eliminations.
...
# use in 3-analysis.R.
#
# FA10a vs FA10b (controlled by ln_transform in fa-constants.R):
...
#
# The ln_transform flag is set at the Step 8 checkpoint in fa-constants.R.
