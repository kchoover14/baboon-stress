...
#This script examines measurement error in male maxillary lengths

library(readxl)
library(dplyr)
library(tidyr)
me=read_excel("0-reps-10.xlsx", sheet="malelmx")
...
write.table(mxm1,"mxm1-malel.txt",sep="\t")
...
write.table(mxm2,"mxm2-malel.txt",sep="\t")
...
write.table(mxm3,"mxm3-malel.txt",sep="\t")
...
write.table(mxp4,"mxp4-malel.txt",sep="\t")

#Rosner's outlier detection
library(EnvStats)
rosnerTest(mxm1$mxm1, k=10, alpha=0.05, warn=TRUE)
rosnerTest(mxm2$mxm2, k=10, alpha=0.05, warn=TRUE)
rosnerTest(mxm3$mxm3, k=10, alpha=0.05, warn=TRUE)
rosnerTest(mxp3$mxp3, k=10, alpha=0.05, warn=TRUE)
